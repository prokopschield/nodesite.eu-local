...
import { ListenerResponse, NodeSiteClient, NodeSiteRequest, rewrite } from 'nodesite.eu';
...
export function listen (options: NSLocalOptions) {
	if (options.interface === 'https' && (!options?.certificate?.cert || !options?.certificate?.key)) {
		throw new Error(`Either supply path to certificate, or use http!`);
	}
	if (!options.name.includes('.')) options.name = `${options.name}.nodesite.eu`;
	...
	const server = (options.interface === 'https') ? (
		https.createServer({
			cert,
			key,
		}, listener)
	...
	return ({
		server,
		create: (path: string, listener?: null | undefined | ((req: NodeSiteRequest) => ListenerResponse | Promise<ListenerResponse>), file?: string) => NodeSiteClient.create(options.name, path, listener, file),
	});
}
